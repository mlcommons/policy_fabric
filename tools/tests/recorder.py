...
look identical afterwards. This gives the browser a virtual X display, points
ffmpeg at that display, and lets it run from the first click to the last. The
test never stops to take a picture, so nothing about its timing changes and
nothing between two steps is missed.

The caption is a banner injected into the page rather than drawn into the video:
no filter graph, and it lands in the failure screenshots too. It is
...
registered with the browser as well as with the page: Chrome re-runs it on every
document it opens from then on. ``tick()`` is the fallback for a browser that
would not take that registration.

Nothing here may fail a test. A recorder that cannot record says so and stays out
of the way.
"""
...
BANNER = """
function __pdoBanner(text) {
    if (!document.body) { return false; }
    var el = document.getElementById('__pdo_recorder__');
    if (!el) {
        el = document.createElement('div');
        el.id = '__pdo_recorder__';
        el.style.cssText = 'position:fixed;left:0;right:0;bottom:0;z-index:2147483647;'
            + 'pointer-events:none;background:rgba(12,32,38,0.94);color:#fff;'
...
SHOW = BANNER + "\nreturn __pdoBanner(arguments[0]);"

ON_LOAD = BANNER + """
(function () {
    function draw() { __pdoBanner(CAPTION); }
...
class VirtualDisplay:
    ...
    def start(self):
        ...
        for number in range(self.first, self.last):
            if os.path.exists(self._socket(number)):
                continue
            self.number = number
    ...
    def _wait(self, timeout=10):
        ...
        while time.time() < deadline:
            if os.path.exists(self._socket(self.number)):
                return True
...
class Recorder:
    ...
    def stop(self):
        ...
        if self.process.returncode not in (0, 255):
            print(f"    (video not written: {errors.decode()[:400].strip()})", flush=True)
            return None
