...
Run the CLI test in the client container. Mounts the host files and invokes
the same scripts/run_cli.sh inside the container.
...
# The ENTRYPOINT is bash; we pass the runner + args as the CMD, with
...
docker run --rm --name policies_client_container \
    ...
    $PDO_CLIENT_IMAGE /scripts/run_cli.sh \
    --cert-path /tmp/cert.pem \
    --site-toml /tmp/site.toml \
    --host "$F_SERVICE_HOST" \
    --ledger-url "$PDO_LEDGER_URL" \
    --eservice-url "$PREFERRED_ESERVICE_URL"
