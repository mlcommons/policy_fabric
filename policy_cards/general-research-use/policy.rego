...
#
# Allow when the requester has accepted the data owner's required terms document and
# presents a channel public key. An AgreementCredential whose claim
...
# subject, so the requester both accepts the terms and owns the channel key. That
# key is returned as the parameters of a "do_download" operation so the token can
# build the guardian capability. Signature verification of the credentials the
# decision relies on is delegated to the rego_policy_agent contract via
# `verification_tasks` (referenced by global index).
#
...
requirements := {"User": ["AgreementCredential", "publicKeyCredential"]}
...
# an accepted terms document AND a public key credential issued to that subject
...
# ---- verification tasks ---------------------------------------------------
# Flag the same-subject credentials the decision relies on (the matching agreement
# and the public key) for the contract to verify by global index.
