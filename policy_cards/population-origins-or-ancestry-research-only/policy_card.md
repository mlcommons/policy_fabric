# Policy Card — DUO_0000011 POA (Population Origins or Ancestry Research Only)
...
- **id:** `card.duo.0000011-poa`
- **version:** 1.0.0
- **status:** released
- **title:** Population Origins or Ancestry Research Only (DUO_0000011 POA)
- **description:** Grant access only when a project's intended-use approval is a
  permitted population-origins or ancestry research purpose, and the requester is
  bound to that project as a member of the owning PI's team.
- **author:** MLCommons
- **contact:** —
...
|---------|------------|-------------------------------|----------|--------------------------------------------------------|
| 1.0.0   | 2026-07-15 | MLCommons                     | released | Initial policy card authored from the POA policy.      |

## 4  Summary & Intent

Access to the dataset is limited to population-origins and ancestry research. A
project's `IntendedDataUseCredential` declares the purposes the project is
approved to pursue; access is granted only when at least one of those purposes is
in the `allowedPurposes` set the data owner configured. The requester is tied to
...
- **Governance objective:** restrict download of the asset to projects whose
  approved intended use is a permitted population-origins or ancestry research
  purpose.
...
intended-use purposes include an allowed purpose and the ownership/team chain
