# DUO_0000011 — POA (Population Origins or Ancestry Research Only)
#
# Allow when a project's IntendedDataUseCredential declares a purpose within the
# data owner's allowed population-origins/ancestry research scope, and the requester
# is bound to that project through the team/ownership chain: a TeamCredential places
# the requester on a PI's team, and a ProjectOwnershipCredential shows that PI owns
# the project the IDU is about. The requester must also present an AgreementCredential
# (accepted terms) and a publicKeyCredential whose claim is the requester's channel
# public key. The allowed purpose terms are configured by the data owner in
...
#   input.policy_data         : { allowedPurposes: [..] }
...
# intended-data-use credentials whose purposes indicate population-origins/ancestry research
matching_idu := [c |
	some c in idu_creds
