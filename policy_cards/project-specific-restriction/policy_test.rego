# Unit tests for DUO_0000027 (PS).  Run with:  opa test ps/ -v
package subpolicy_test

import data.subpolicy

base_input := {
	"policy_data": {"approvedProjects": ["did:example:project-1"]},
	"presentations": {"User": [
		{
...
test_allow_when_project_approved if {
...
test_deny_when_project_not_approved if {
	other := json.patch(base_input, [{
		"op": "replace",
		"path": "/presentations/User/0/subject",
		"value": "did:example:project-unapproved",
	}])
