# Policy Card — DUO_0000027 PS (Project Specific Restriction)
...
- **id:** `card.duo.0000027-ps`
- **version:** 1.0.0
- **status:** released
- **title:** Project Specific Restriction (DUO_0000027 PS)
- **description:** Grant access only when a project's intended-use approval is for
  a project on the data owner's approved list, and the requester is bound to that
...
|---------|------------|-------------------------------|----------|--------------------------------------------------------|
| 1.0.0   | 2026-07-15 | MLCommons                     | released | Initial policy card authored from the PS policy.       |

## 4  Summary & Intent

Access to the dataset is limited to projects on the data owner's approved list. A
project's `IntendedDataUseCredential` is issued to the project as its subject;
access is granted only when that project identifier is in the `approvedProjects`
set the data owner configured. The requester is tied to the project through a
chain: their `TeamCredential` names the PI whose `ProjectOwnershipCredential`
names the project the `IntendedDataUseCredential` is about. The requester's
`TeamCredential`, `AgreementCredential`, and `publicKeyCredential` are issued to
the **same subject**, so a single party is on the team, has accepted the terms,
and owns the key the data is encrypted to.

- **Governance objective:** restrict download of the asset to projects on the data
  owner's approved list.
...
Permission: download the target dataset
  ...
  Conditions (all must hold):
    - an IntendedDataUseCredential's subject (the project) ∈ approvedProjects
...
|------------------------------|--------------------------|----------|
| `IntendedDataUseCredential`  | the project identifier (credential subject) | yes      |
...
```json
{
  "approvedProjects": ["list of approved project DIDs"]
}
```

`approvedProjects` is the set of project identifiers (the `subject` of an
`IntendedDataUseCredential`) for which access is permitted.
...
intended-use credential is for an approved project and the ownership/team chain
