# Unit tests for DUO_0000018 (NPUNCU).  Run with:  opa test npuncu/ -v
package subpolicy_test

import data.subpolicy

base_input := {
	"policy_data": {"nonprofitLegalForms": ["V2VJ", "8ES1"], "prohibitedPurposes": ["commercial"]},
	"presentations": {"User": [
		{
...
test_requirements_are_user_role_with_credential_types if {
	subpolicy.requirements == {"User": ["LegalDesignationCredential", "IntendedDataUseCredential", "publicKeyCredential"]}
}

test_allow_when_nonprofit_and_non_commercial if {
...
test_tasks_flag_legal_idu_and_public_key if {
...
test_deny_when_intended_use_is_commercial if {
	commercial := json.patch(base_input, [{
		"op": "replace",
		"path": "/presentations/User/1/claims/useOnlyFor/purposes",
		"value": ["commercial"],
	}])
	subpolicy.result.decision == false with input as commercial
}
