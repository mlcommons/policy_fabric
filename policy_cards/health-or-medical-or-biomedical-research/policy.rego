# DUO_0000006 — HMB (Health or Medical or Biomedical Research)
#
# Allow when a project's IntendedDataUseCredential declares a purpose within the
# data owner's allowed health/medical/biomedical scope and none of the prohibited
# population-origins/ancestry purposes, and the requester is bound to that project
# through the team/ownership chain: a TeamCredential places the requester on a PI's
# team, and a ProjectOwnershipCredential shows that PI owns the project the IDU is
# about. The requester must also present an AgreementCredential (accepted terms) and
# a publicKeyCredential whose claim is the requester's channel public key. The
# allowed and prohibited purpose terms are configured by the data owner in
# policy_data. That key is returned as the parameters of a "do_download" operation
# so the token can build the guardian capability. Signature verification of the
# credentials the decision relies on is delegated to the rego_policy_agent contract
# via `verification_tasks` (referenced by global index).
#
...
#   input.policy_data         : { allowedPurposes: [..], prohibitedPurposes: [..] }
...
idu_has_allowed_purpose(c) if {
	some p in c.claims.useOnlyFor.purposes
	p in input.policy_data.allowedPurposes
}

idu_has_prohibited_purpose(c) if {
	some p in c.claims.useOnlyFor.purposes
	p in input.policy_data.prohibitedPurposes
}

# intended-data-use credentials whose purposes include an allowed health/medical/
# biomedical purpose and none of the prohibited population-origins/ancestry purposes
matching_idu := [c |
	some c in idu_creds
	idu_has_allowed_purpose(c)
	not idu_has_prohibited_purpose(c)
]
