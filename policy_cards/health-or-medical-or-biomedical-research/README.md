...
Access is limited to projects whose approved intended use is a permitted health,
medical, or biomedical purpose and is not a prohibited population-origins or
ancestry purpose. The requester presents the project's
...
`key`) carrying their channel public key. The data owner configures the
`allowedPurposes` and `prohibitedPurposes` sets in `policy_data`. Access is
**allowed** when at least one of the project's declared purposes is in the allowed
set, none is in the prohibited set, and the chain resolves to a single requester;
otherwise it is **denied**. The credentials the decision relies on are emitted as
...
**Policy data:** `allowedPurposes: [..], prohibitedPurposes: [..]`
