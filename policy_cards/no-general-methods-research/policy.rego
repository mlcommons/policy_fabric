# DUO_0000015 — NMDS (No General Methods Research)
#
# Allow when a project's IntendedDataUseCredential declares none of the data owner's
# prohibited methods-development, AI/software-development, or model-training purposes,
# and the requester is bound to that project through the team/ownership chain: a
# TeamCredential places the requester on a PI's team, and a ProjectOwnershipCredential
# shows that PI owns the project the IDU is about. The requester must also present an
# AgreementCredential (accepted terms) and a publicKeyCredential whose claim is the
# requester's channel public key. The prohibited purpose terms are configured by the
# data owner in policy_data. That key is returned as the parameters of a "do_download"
# operation so the token can build the guardian capability. Signature verification of
# the credentials the decision relies on is delegated to the rego_policy_agent contract
...
#   input.policy_data         : { prohibitedPurposes: [..] }
...
# intended-data-use credentials that do not declare a methods-development,
# AI/software-development, or model-training purpose
matching_idu := [c |
	some c in idu_creds
