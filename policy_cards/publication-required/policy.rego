# DUO_0000019 — PUB (Publication Required)
#
# Allow when an executed publication agreement binds the requester's institution to
# the governed dataset. The requester is tied to a project and institution through a
# chain: a TeamCredential places them on a PI's team, a ProjectOwnershipCredential
# shows that PI owns a project, and an AffiliationCredential shows that PI belongs to
# an institution. A ScopedAgreementCredential names a terms document scoped to that
# project with the "Publication" obligation for the governed dataset, and an
# AgreementCredential shows the institution has agreed to that terms document. The
# requester also presents a publicKeyCredential whose claim is their channel public
# key. The governed dataset is configured by the data owner in policy_data. That key
# is returned as the parameters of a "do_download" operation so the token can build
# the guardian capability. Signature verification of the credentials the decision
# relies on is delegated to the rego_policy_agent contract via `verification_tasks`
...
# scoped-agreement documents carrying the publication obligation for the governed dataset
scoped_for_publication := [c |
	some c in scoped_agreement_creds
	c.claims.agreementInfo.scope.obligation == "Publication"
...
# publication agreement for the governed dataset:
...
#   ScopedAgreementCred      terms doc   -> project + publication obligation
...
qualified_chains := {chain |
	...
	some scoped in scoped_for_publication
