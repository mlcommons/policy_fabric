# Unit tests for DUO_0000007 (DS).  Run with:  opa test ds/ -v
package subpolicy_test

import data.subpolicy

base_input := {
	"policy_data": {"allowedDiseases": ["MONDO:0005148", "MONDO:0005015"]},
	"presentations": {"User": [
		{
			"type": "IntendedDataUseCredential",
			"issuer": "did:example:dac",
			"subject": "did:example:project-1",
			"claims": {"useOnlyFor": {"purposes": ["HMB"], "diseases": ["MONDO:0005148"]}},
			"index": 0,
		},
		{
...
test_allow_when_disease_in_scope if {
...
test_deny_when_disease_not_in_scope if {
	other := json.patch(base_input, [{
		"op": "replace",
		"path": "/presentations/User/0/claims/useOnlyFor/diseases",
		"value": ["MONDO:9999999"],
	}])
