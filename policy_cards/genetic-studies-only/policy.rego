# DUO_0000016 — GSO (Genetic Studies Only)
#
# Allow when a project's IntendedDataUseCredential declares a purpose within the
# data owner's allowed genetic/genotype study scope, and the requester is bound to
# that project through the team/ownership chain: a TeamCredential places the
# requester on a PI's team, and a ProjectOwnershipCredential shows that PI owns the
# project the IDU is about. The requester must also present an AgreementCredential
...
# intended-data-use credentials whose purposes indicate a genetic/genotype study
matching_idu := [c |
	some c in idu_creds
