...
# prohibited population-origins/ancestry purposes, and the requester is bound to that
# project through the team/ownership chain: a TeamCredential places the requester on a
# PI's team, and a ProjectOwnershipCredential shows that PI owns the project the IDU is
...
# prohibited purpose terms are configured by the data owner in policy_data. That key is
...
# intended-data-use credentials that do not declare a population-origins/ancestry purpose
matching_idu := [c |
	some c in idu_creds
