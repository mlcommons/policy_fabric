# DUO_0000046 — NCU (Non-Commercial Use Only)
#
# Allow when a project's IntendedDataUseCredential declares none of the data owner's
# prohibited commercial purposes, and the requester is bound to that project through
# the team/ownership chain: a TeamCredential places the requester on a PI's team, and
# a ProjectOwnershipCredential shows that PI owns the project the IDU is about. The
# requester must also present an AgreementCredential (accepted terms) and a
# publicKeyCredential whose claim is the requester's channel public key. The
# prohibited purpose terms are configured by the data owner in policy_data. That key
